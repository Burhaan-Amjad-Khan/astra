...
def safe_plotly(fig_or_func, title="Chart"):
    try:
...
if not st.session_state.data_ready:
    ...
    if page == "Mission Overview":
        ...
        with col1:
            safe_plotly(
                lambda: StatusCharts.gauge_chart(avg_health, "Fleet Average Health"),
                "Fleet Health Gauge",
            )
        with col2:
            dist_data = {"Excellent": excellent, "Good": good, "Warning": warning, "Critical": critical}
            safe_plotly(
                lambda: CircularCharts.donut_chart(dist_data, "Health Distribution", f"{avg_health}%"),
                "Health Distribution",
            )
        with col3:
            health_scores_map = {h["satellite_id"]: h["score"] for h in fleet_health[:10]}
            safe_plotly(
                lambda: StatusCharts.progress_bars(health_scores_map, "Top 10 Health Scores"),
                "Health Scores",
            )
        with col4:
    ...
    # ── ANALYTICS ──
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.plotly_chart(DistributionCharts.histogram(sat_df, "battery_pct", title="Battery Distribution"),
                            width='stretch')
        with c2:
            st.plotly_chart(DistributionCharts.histogram(sat_df, "temperature_c", title="Temperature Distribution"),
                            width='stretch')
